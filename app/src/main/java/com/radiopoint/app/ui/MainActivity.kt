...
class MainActivity:AppCompatActivity(),LocationListener {
    ...
    override fun onCreate(savedInstanceState:Bundle?) {
        ...
        binding.btnToggleLoadStatus.setOnClickListener {status=VehicleStatus.entries[(status.ordinal+1)%VehicleStatus.entries.size];prefs.edit().putInt("status",status.code).apply();refresh()}
        ...
        binding.btnMarkPullout.setOnClickListener { markPullout() }
    ...
    private fun chirp() {
        ...
        withPermissions {
            val road=roadManager.selectedRoad
            val payload=TelemetryPacket(unit,status,if(fix.hasSpeed()&&fix.speed>=0.84)direction else Direction.PARKED,
    ...
    private fun refresh() {
        ...
        binding.btnToggleLoadStatus.text=status.label;binding.btnDirection.text=direction.label
        ...
        binding.txtMapNote.text="$mapName\nNo reports does not mean the road is clear."
        val mapKey="${road.roadCode}:${activeMaps.activePullouts.size}"
        ...
        if(fix!=null) {
            ...
            nearRoad=!road.isDemo && roadManager.isPointNearRoad(localUtm,150.0)
            marker(fix.latitude,fix.longitude,"Unit $unit · you","GPS accuracy ±${fix.accuracy.roundToInt()} m",R.drawable.ic_truck,if(fresh==null)0.4f else 1f)
        ...
        rows.forEach {r ->val p=r.packet;val age=age(r)
            marker(p.latitude,p.longitude,"Unit ${p.unitId} · ${p.status.label}","${ageLabel(age)} · ±${p.accuracyMeters} m",R.drawable.ic_truck,if(age>30)0.35f else 1f)
            if(fresh!=null && nearRoad && localUtm!=null && p.roadCode==road.roadCode && age<=30 && p.accuracyMeters<=100) {
    ...
    private fun age(r:ReceivedPosition)=((System.currentTimeMillis()-r.receivedAt)/1000).coerceAtLeast(0)+r.packet.fixAgeSeconds
    private fun ageLabel(s:Long)=if(s<60)"${s}s ago" else "${s/60}m ago · stale"
